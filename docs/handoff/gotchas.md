...
**Cause:** Every commit was rewritten with `git filter-repo --mailmap` to strip a stale
author identity (details kept outside this repo), then force-pushed. The repo
was rewritten more than once that day, because a later rewrite in a sibling repo
cascaded hash-reference repairs back through here.
...
`git rev-list --branches --remotes`. Full record:
...
`IDENTITY_GUARD=off git <command>` if the guard is genuinely wrong. Details:
`documented where the guard is installed`.
